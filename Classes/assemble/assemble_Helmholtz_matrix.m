...
sigmaPML_x = @(p) speed(p).*(1./(p(:,1)-xmin + 10*eps ).*(p(:,1) < xmin + wpml(1)) + ...
                1./(xmax-p(:,1)  + 10*eps ).*(p(:,1) > xmax - wpml(2))) ;         
sigmaPML_y = @(p) speed(p).*( 1./(p(:,2)-ymin  + 10*eps ).*(p(:,2) < ymin + wpml(3)) ...
                + 1./(ymax-p(:,2) + 10*eps ).*(p(:,2) > ymax - wpml(4)));
