function A = assemble_Helmholtz_matrix_with_multiple_rays(node,elem,omega,wpml,sigmaMax,speed,ray,fquadorder)
...
PXY = zeros(NT,2*nQuad);
K2 =  zeros(NT, nQuad);
SX =  zeros(NT, nQuad);
SY =  zeros(NT, nQuad);
SXY = zeros(NT, nQuad);

% we perform the evaluation of the function in 
% vectorized form and we store the values 
for p=1:nQuad 
        pxy = lambda(p,1)*node(elem(:,1),:) ...
            + lambda(p,2)*node(elem(:,2),:) ...
            + lambda(p,3)*node(elem(:,3),:);
        K2(:,p) = (omega./speed(pxy)).^2;
