...
% sigmaPML_x = @(p) sigmaMax*( (p(:,1)-xmin-wpml(1)).^2.*(p(:,1) < xmin + wpml(1))/wpml(1)^2 + ...
%                 (p(:,1)-(xmax-wpml(2))).^2.*(p(:,1) > xmax - wpml(2))/wpml(2)^2);         
% sigmaPML_y = @(p) sigmaMax*( (p(:,2)-ymin-wpml(3)).^2.*(p(:,2) < ymin + wpml(3))/wpml(3)^2 ...
